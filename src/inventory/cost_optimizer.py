"""
src/inventory/cost_optimizer.py
Cost Optimization Engine for RetailMind-X
...
class InventoryCostOptimizer:
    ...
    def __init__(
        ...
        """
        holding_rate_annual: Annual inventory holding cost as % of unit value (default: 20%)
        fixed_order_cost: Estimated fixed cost per replenishment purchase order (default: $50.00)
        stockout_penalty_multiplier: Penalty for stockout expressed as multiplier of margin/unit price (default: 1.5x)
        overstock_markdown_loss_rate: Estimated depreciation/discount loss on excess inventory (default: 30%)
        """
    ...
    def calculate_costs(
        ...
        Returns:
            - breakdown: holding_cost, ordering_cost, stockout_cost, overstock_cost, total_cost
            - strategy_comparison: current vs recommended (with estimated savings)
            - assumptions: clearly declared configurable assumptions
            - item_cost_details: top items driving cost
        """
        ...
        for item in inventory_items:
            ...
            # Unit price: try item's unit_price, or lookup, or standard default assumption
            unit_price = float(item.get("unit_price", 0) or unit_price_lookup.get(sku, 15.0))
            if unit_price <= 0:
                unit_price = 15.0

            # Annual holding cost per unit = unit_price * holding_rate_annual
            # Monthly holding cost factor = holding_rate_annual / 12
            ...
            # Recommended holding cost (based on balanced safety stock + half cycle inventory)
            ...
            # Ordering cost
            # Current: if reorder needed, incurring order cost
            ...
            # Stockout risk cost (penalty * expected lost sales)
            ...
            # Recommended mitigates stockout risk through timely buffer
            ...
            if overstock_risk or (current_stock > safety_stock * 3 and safety_stock > 0):
                ...
                c_overstock = 0.0
            r_overstock = c_overstock * 0.20  # Optimized reorder targets prevent excess accumulation
