"""
src/intelligence/decision_center.py
AI Decision Center for RetailMind-X
...
100% session-grounded; no synthetic/fabricated recommendations.
"""
...
class AIDecisionCenter:
    ...
    def generate_decision_dashboard(
        ...
        unit_label = target_display or "units"
        ...
        for item in inventory_items:
            ...
            if risk == "CRITICAL" or (stock == 0 and reorder_qty > 0):
                ...
                actions.append({
                    ...
                    "priority": "CRITICAL",
                    "what": f"Current stock is {int(stock)} {unit_label}. Buffer is exhausted.",
                    "why": f"Safety threshold is {int(safety)} {unit_label}. Immediate demand risk detected with lead time of {lead_time} days.",
                    "action": f"Issue emergency Purchase Order for {int(reorder_qty)} {unit_label} immediately to avoid lost revenue.",
                ...
                actions.append({
                    ...
                    "priority": "HIGH",
                    "what": f"Stock level ({int(stock)} {unit_label}) has dipped below reorder point.",
                    "why": f"Safety stock is {int(safety)} {unit_label} and predicted demand will deplete remaining inventory within normal lead time.",
                    "action": f"Place standard replenishment order for {int(reorder_qty)} {unit_label}.",
                ...
                actions.append({
                    ...
                    "priority": "MEDIUM",
                    "what": f"Holding {int(stock)} {unit_label} exceeds 2.5x buffer requirements ({int(safety)} {unit_label}).",
        ...
        # Rank actions by priority: CRITICAL > HIGH > MEDIUM > LOW
